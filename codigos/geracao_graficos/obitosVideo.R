...
setwd('C:\\Users\\gvheisler\\Desktop\\plots')
...
for(dia in ds$data){
  ...
  png(paste("obitos-", dia, ".png", sep = ''))
  plot(x = dsAux$data, y = dsAux$quant_obitos, type = 'l', xlim = c(min(ds$data), max(ds$data)),
       ylim = c(min(ds$quant_obitos), max(ds$quant_obitos)), lwd = 1, 
       ylab = "Quantidade de óbitos", xlab = "Data", main = "Evolução da COVID-19 no RS")
  dev.off()
}
